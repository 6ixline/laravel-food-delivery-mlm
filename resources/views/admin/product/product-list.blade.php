...
@section('title')
    <title>Products ({{$kitchen->name}})</title>
@endsection
@section('content')
<!-- CONTAINER -->
<div class="main-container container-fluid">
    ...
    <div class="page-header mt-2">
        <h1 class="page-title">Food Items ({{$kitchen->name}}) <a href="{{route("admin.product", ['view' => 'form', 'mode'=> 'insert', 'kitchen_id'=> $kitchen->id])}}" class="btn btn-primary btn-sm mx-3 mb-2">Add New</a></h1>
        <div>
            <ol class="breadcrumb">
    ...
    <form name="form_actions" method="POST" action="{{ route("product.actions") }}" ENCTYPE="MULTIPART/FORM-DATA">
        @csrf
         <div class="row">
            <div class="col-sm-12 mb-0">
                <div class="form-inline">
                    ...
                    <input type="submit" value="Filter" class="btn btn-primary btn-sm mb-2 mx-2" /><a href="{{route("admin.product", ['view' => 'list', 'kitchen_id'=> $kitchen->id])}}" class="btn btn-danger btn-sm mb-2">Clear</a> 
                </div>
            </div>
        </div>
                            ...
                            <table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="wd-15p border-bottom-0">Variant</th>
                                ...
                                @if(count($productRows) >= 1)
                                    @foreach($productRows as $productRow)
                                        <tr>
                                            <td><input type="checkbox" name="del_items[]" value="{{ $productRow['id'] }}" /></td>
                                            <td>{{ $productRow['name'] }}</td>
                                            <td>{{ $productRow->category->title ?? 'N/A' }}</td>
                                           
                                            <td>
                                                @if ($productRow['pricing_mode'] == 'variant')
                                                  <a class="btn btn-primary btn-sm" href="{{route("admin.productVariant", ['view' => 'list', 'productid'=> $productRow['id'], 'kitchen_id'=> $kitchen->id])}}" > Manage Variant</a>  
                                                @else -- @endif
                                            </td>
                                             <td><span class="badge text-light rounded-pill bg-{{ $productRow['status'] == 'active' ? 'success' : 'danger' }} badge-sm me-1 mb-1 mt-1">{{ ucfirst($productRow['status']) }}</span></td>
                                           
                                            <td>
                                                {{ \Carbon\Carbon::parse($productRow['created_at'])->format('d M, Y') }} 
                                                <br class="mb-hidden" />
                                                ({{ \Carbon\Carbon::parse($productRow['created_at'])->diffForHumans() }})
                                            </td>
                                           
                                            <td>
                                                <div class="g-2">
                                                  
                                                        <a class="btn text-primary btn-sm" href="{{route("admin.product", ['view' => 'form', 'mode'=> 'edit', 'productid'=> $productRow['id'], 'kitchen_id'=> $kitchen->id])}}" data-bs-toggle="tooltip" data-bs-original-title="Edit"><span class="fe fe-edit fs-14"></span></a>
