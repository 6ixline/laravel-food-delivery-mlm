...
<div class="main-container container-fluid">
    ...
    <div class="page-header">
        <h1 class="page-title">Pincodes <a href="{{route("admin.pincodeMaster", ['view' => 'form', 'mode'=> 'insert'])}}" class="btn btn-primary btn-sm mx-3 mb-2">Add New</a></h1>
        <div>
            <ol class="breadcrumb">
    ...
    <form name="form_actions" method="POST" action="{{ route("pincodeMaster.actions") }}" ENCTYPE="MULTIPART/FORM-DATA">
        @csrf
         <div class="row">
            <div class="col-sm-12 mb-0">
                <div class="form-inline">
                    ...
                    <input type="submit" value="Filter" class="btn btn-primary btn-sm mb-2 mx-2" /><a href="{{route("admin.pincodeMaster", ['view' => 'list'])}}" class="btn btn-danger btn-sm mb-2">Clear</a> 
                </div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                                ...
                                @if(count($pincodeMasterRows) >= 1)
                                    @foreach($pincodeMasterRows as $pincodeMasterRow)
                                        <tr>
                                            <td><input type="checkbox" name="del_items[]" value="{{ $pincodeMasterRow['id'] }}" /></td>
                                            <td>{{ $pincodeMasterRow['pincode'] }}</td>
                                            <td>{{ $pincodeMasterRow['area'] }}</td>
                                            <td>{{ $pincodeMasterRow['city'] }}</td>
                                            <td>{{ $pincodeMasterRow['state'] }}</td>
                                            <td>{{ $pincodeMasterRow['country'] }}</td>
                                            <td><span class="badge rounded-pill bg-{{ $pincodeMasterRow['status'] == 'active' ? 'success' : 'danger' }} badge-sm me-1 mb-1 mt-1">{{ ucfirst($pincodeMasterRow['status']) }}</span></td>
                                            <td>
                                                {{ \Carbon\Carbon::parse($pincodeMasterRow['created_at'])->format('d M, Y') }} 
                                                <br class="mb-hidden" />
                                                ({{ \Carbon\Carbon::parse($pincodeMasterRow['created_at'])->diffForHumans() }})
                                            </td>
                                            <td>
                                                <div class="g-2">
                                                  
                                                        <a class="btn text-primary btn-sm" href="{{route("admin.pincodeMaster", ['view' => 'form', 'mode'=> 'edit', 'pinid'=> $pincodeMasterRow['id']])}}" data-bs-toggle="tooltip" data-bs-original-title="Edit"><span class="fe fe-edit fs-14"></span></a>
                                                        {{-- |
                                                        <form action="{{ route('pincodeMaster.delete') }}" name="single-delete" method="POST" style="display:inline;" onsubmit="event.stopPropagation();">
                                                            @csrf
                                                            <input type="hidden" name="id" value="{{ $pincodeMasterRow['id'] }}">
